...
public class CreatePolicyDraftHandlerTests
{
    ...
    public async Task Handle_ShouldCreateDraftAndReturnDetails()
    {
        ...
        var capturedVersion = capturedPolicy.PolicyVersions.Should().ContainSingle().Subject;
        capturedVersion.IsActiveVersion.Should().BeTrue();
        capturedVersion.PolicyAdjustments.Should().HaveCount(1);
        capturedVersion.PolicyAdjustments.All(a => a.PolicyVersion == capturedVersion).Should().BeTrue();
    }
