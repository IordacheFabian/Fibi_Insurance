...
public class ReportsController_Tests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task PoliciesByCountry_Should_ReturnGroupedTotals()
    {
        ...
        var seeded = await SeedDefaultPoliciesAsync();

        var from = new DateOnly(2025, 12, 1);
        var to = new DateOnly(2026, 12, 31);

        var response = await GetReportAsync<PoliciesByCountryListDto>(CountryPath, from, to);
        var expected = BuildExpectedByCountry(FilterPolicies(seeded, from, to).ToList());

        AssertPagedResult(response, expected.Count, 1, 50);
        AssertCountryRows(ToListOrEmpty(response.Items), expected);
    }
    ...
    public async Task PoliciesByCounty_Should_ReturnGroupedTotals()
    {
        ...
        var response = await GetReportAsync<PoliciesByCountyListDto>(CountyPath, from, to);
        var expected = BuildExpectedByCounty(FilterPolicies(seeded, from, to).ToList());

        AssertPagedResult(response, expected.Count, 1, 50);
        AssertCountyRows(ToListOrEmpty(response.Items), expected);
    }
    ...
    public async Task PoliciesByCity_Should_ReturnGroupedTotals()
    {
        ...
        var response = await GetReportAsync<PoliciesByCityListDto>(CityPath, from, to);
        var expected = BuildExpectedByCity(FilterPolicies(seeded, from, to).ToList());

        AssertPagedResult(response, expected.Count, 1, 50);
        AssertCityRows(ToListOrEmpty(response.Items), expected);
    }
    ...
    public async Task PoliciesByBroker_Should_ReturnGroupedTotals()
    {
        ...
        var response = await GetReportAsync<PoliciesByBrokerListDto>(BrokerPath, from, to);
        var expected = BuildExpectedByBroker(FilterPolicies(seeded, from, to).ToList());

        AssertPagedResult(response, expected.Count, 1, 50);
        AssertBrokerRows(ToListOrEmpty(response.Items), expected);
    }
    ...
    public async Task PoliciesByCountry_Should_ApplyFilters()
    {
        ...
        await RunFilterMatrixAsync(CountryPath, seeded, BuildExpectedByCountry, AssertCountryRows);
    }
    ...
    public async Task PoliciesByCounty_Should_ApplyFilters()
    {
        ...
        await RunFilterMatrixAsync(CountyPath, seeded, BuildExpectedByCounty, AssertCountyRows);
    }
    ...
    public async Task PoliciesByCity_Should_ApplyFilters()
    {
        ...
        await RunFilterMatrixAsync(CityPath, seeded, BuildExpectedByCity, AssertCityRows);
    }
    ...
    public async Task PoliciesByBroker_Should_ApplyFilters()
    {
        ...
        await RunFilterMatrixAsync(BrokerPath, seeded, BuildExpectedByBroker, AssertBrokerRows);
    }
    ...
    public async Task PoliciesByCountry_Should_ReturnEmptyList_WhenFiltersExcludeAll()
    {
        ...
        var outsideRange = await GetReportAsync<PoliciesByCountryListDto>(CountryPath, new DateOnly(2024, 1, 1), new DateOnly(2024, 12, 31));
        AssertPagedResult(outsideRange, 0, 1, 50);
        Assert.Empty(ToListOrEmpty(outsideRange.Items));

        var missingCurrency = await GetReportAsync<PoliciesByCountryListDto>(CountryPath, new DateOnly(2026, 1, 1), new DateOnly(2026, 3, 31), currency: "USD");
    ...
    public async Task PoliciesByCounty_Should_ReturnEmptyList_WhenFiltersExcludeAll()
    {
        ...
        var outsideRange = await GetReportAsync<PoliciesByCountyListDto>(CountyPath, new DateOnly(2024, 1, 1), new DateOnly(2024, 12, 31));
        AssertPagedResult(outsideRange, 0, 1, 50);
        Assert.Empty(ToListOrEmpty(outsideRange.Items));

        var missingCurrency = await GetReportAsync<PoliciesByCountyListDto>(CountyPath, new DateOnly(2026, 1, 1), new DateOnly(2026, 3, 31), currency: "USD");
        AssertPagedResult(missingCurrency, 0, 1, 50);
        Assert.Empty(ToListOrEmpty(missingCurrency.Items));
    }
    ...
    public async Task PoliciesByCity_Should_ReturnEmptyList_WhenFiltersExcludeAll()
    {
        ...
        var outsideRange = await GetReportAsync<PoliciesByCityListDto>(CityPath, new DateOnly(2024, 1, 1), new DateOnly(2024, 12, 31));
        AssertPagedResult(outsideRange, 0, 1, 50);
        Assert.Empty(ToListOrEmpty(outsideRange.Items));

        var missingCurrency = await GetReportAsync<PoliciesByCityListDto>(CityPath, new DateOnly(2026, 1, 1), new DateOnly(2026, 3, 31), currency: "USD");
        AssertPagedResult(missingCurrency, 0, 1, 50);
        Assert.Empty(ToListOrEmpty(missingCurrency.Items));
    }
    ...
    public async Task PoliciesByBroker_Should_ReturnEmptyList_WhenFiltersExcludeAll()
    {
        ...
        var outsideRange = await GetReportAsync<PoliciesByBrokerListDto>(BrokerPath, new DateOnly(2024, 1, 1), new DateOnly(2024, 12, 31));
        AssertPagedResult(outsideRange, 0, 1, 50);
        Assert.Empty(ToListOrEmpty(outsideRange.Items));

        var missingCurrency = await GetReportAsync<PoliciesByBrokerListDto>(BrokerPath, new DateOnly(2026, 1, 1), new DateOnly(2026, 3, 31), currency: "USD");
        AssertPagedResult(missingCurrency, 0, 1, 50);
        Assert.Empty(ToListOrEmpty(missingCurrency.Items));
    }
